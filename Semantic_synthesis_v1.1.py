...
def skip_connection_block(input_a_layer, input_b_layer, resize, filters):
	...
	output_skip_layer = input_a + input_b_layer
...
def residual_block(inputs_layer, nb_blocks, out_filters, strides):
	for _ in range(nb_blocks):
		...
		# 1x1 conv2d_transpose
		# shortcut = tf.layers.conv2d_transpose(inputs = inputs_layer, filters = out_filters, kernel_size = 1, strides = 1, padding='same', data_format='channels_last')
		# inputs_layer = tf.contrib.layers.batch_norm(inputs_layer)
		# inputs_layer = tf.nn.relu(inputs_layer)
...
def VGG16_Synthesis_Network(input_img, input_Semantic):
	...
	output_1 = residual_block(inputs_layer = z, nb_blocks = 2, out_filters = 512, strides = 2)
	# output_1 = skip_connection_block(output_1, vgg.pool5, 7, 512)
	output_2 = tf.layers.conv2d_transpose(inputs = output_1, filters = 256, kernel_size = 3, strides = 2, padding='same', data_format='channels_last')
	...
	# output_2 = skip_connection_block(output_2, vgg.pool4, 14, 256)
	output_3 = residual_block(inputs_layer = output_2, nb_blocks = 1, out_filters = 128, strides = 2)
	# output_3 = skip_connection_block(output_3, vgg.pool3, 28, 128)
	output_4 = tf.layers.conv2d_transpose(inputs = output_3, filters = 64, kernel_size = 3, strides = 2, padding='same', data_format='channels_last')
	output_4 = tf.contrib.layers.batch_norm(output_4)
	output_4 = tf.nn.relu(output_4)
	# output_4 = residual_block(inputs_layer = output_3, nb_blocks = 1, out_filters = 64, strides = 2)
	# output_4 = skip_connection_block(output_4, vgg.pool2, 56, 64)
	output_5 = residual_block(inputs_layer = output_4, nb_blocks = 1, out_filters = 32, strides = 2)
	# output_5 = skip_connection_block(output_5, vgg.pool1, 112, 32)
	output_6 = residual_block(inputs_layer = output_5, nb_blocks = 1, out_filters = 3, strides = 2)
	# output_7 = residual_block(inputs_layer = output_6, nb_blocks = 1, out_filters = 32, strides = 2)
	# output_8 = residual_block(inputs_layer = output_7, nb_blocks = 1, out_filters = 3, strides = 2)
	# output_1 = tf.image.resize_images(z, [7,7])
	# output_1 = skip_connection_block(output_1, vgg.pool5, 512)
	# output_2 = skip_connection_block(output_1, vgg.pool4, 512)
	# output_3 = skip_connection_block(output_2, vgg.pool3, 256)
	# output_4 = skip_connection_block(output_3, vgg.pool5, 128)
	# output_5 = skip_connection_block(output_4, vgg.pool5, 128)

	# output
	output_6 = tf.image.resize_images(output_6, [224,224])
	return output_6
...
checkpoint_dir_1 = 'model_weight_rotation_new_skip_block_7/'
