...
def generator(path):
	...
	for i in range(1000):
		...
		input_b = np.loadtxt(path + 'rendering_little_rotation.txt')
		...
		input_c = utils.load_image_com(path + 'chair_rotation_little_%.2d.png' % (i*1))[:,:,:3]
		for k in range(1,10,1):
			...
			input_c = np.concatenate((input_c, utils.load_image_com(path + 'chair_rotation_little_%.2d.png' % (i*10+k))[:,:,:3]), axis = 0)
...
checkpoint_dir_1 = 'model_weight_rotation_v3.1/'
