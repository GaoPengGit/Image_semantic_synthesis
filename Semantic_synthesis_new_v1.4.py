...
def VGG16_Synthesis_Network(input_img, input_Semantic):
	...
	# Semantic encoder
	Semantic_layer_1_1 = tf.contrib.layers.fully_connected(input_Semantic, 64)
	Semantic_layer_1_1 = tf.contrib.layers.fully_connected(Semantic_layer_1_1, 128)
	Semantic_layer_1_1 = tf.contrib.layers.fully_connected(Semantic_layer_1_1, 256)
	Semantic_layer_1_1 = tf.contrib.layers.fully_connected(Semantic_layer_1_1, 512)
	Semantic_layer_1_1 = tf.contrib.layers.fully_connected(Semantic_layer_1_1, 1225)
	Semantic_layer_1_1 = tf.reshape(Semantic_layer_1_1,[-1,7,7,25])
	...
	# output_1_1 = residual_block(inputs_layer = vgg_1.pool5, nb_blocks = 1, out_filters = 256, strides = 1)
	output_1_1 = tf.layers.conv2d_transpose(inputs = vgg_1.pool5, filters = 256, kernel_size = 3, strides = 1, padding='same', data_format='channels_last', activation = tf.nn.relu)
	Semantic_layer_1_1 = tf.layers.conv2d_transpose(inputs = Semantic_layer_1_1, filters = 32, kernel_size = 3, strides = 1, padding='same', data_format='channels_last', activation = tf.nn.relu)
	Semantic_layer_1_1 = tf.layers.conv2d_transpose(inputs = Semantic_layer_1_1, filters = 64, kernel_size = 3, strides = 1, padding='same', data_format='channels_last', activation = tf.nn.relu)
	Semantic_layer_1_1 = tf.layers.conv2d_transpose(inputs = Semantic_layer_1_1, filters = 128, kernel_size = 3, strides = 1, padding='same', data_format='channels_last', activation = tf.nn.relu)
	Semantic_layer_1_1 = tf.layers.conv2d_transpose(inputs = Semantic_layer_1_1, filters = 256, kernel_size = 3, strides = 1, padding='same', data_format='channels_last', activation = tf.nn.relu)
	...
	output_1_2 = tf.layers.conv2d_transpose(inputs = output_1_1, filters = 256, kernel_size = 3, strides = (2,2), padding='same', data_format='channels_last', activation = tf.nn.relu)
	...
	output_1_2 = tf.layers.conv2d_transpose(inputs = output_1_2, filters = 512, kernel_size = 3, strides = (1,1), padding='same', data_format='channels_last', activation = tf.nn.relu)
	# output_1_3 = residual_block(inputs_layer = output_1_2, nb_blocks = 1, out_filters = 128, strides = 2)
	output_1_3 = tf.layers.conv2d_transpose(inputs = output_1_2, filters = 128, kernel_size = 3, strides = (2,2), padding='same', data_format='channels_last', activation = tf.nn.relu)
	...
	output_1_3 = tf.layers.conv2d_transpose(inputs = output_1_3, filters = 256, kernel_size = 3, strides = (1,1), padding='same', data_format='channels_last', activation = tf.nn.relu)
	# output_1_4 = residual_block(inputs_layer = output_1_3, nb_blocks = 1, out_filters = 64, strides = 2)
	output_1_4 = tf.layers.conv2d_transpose(inputs = output_1_3, filters = 64, kernel_size = 3, strides = (2,2), padding='same', data_format='channels_last', activation = tf.nn.relu)
	...
	output_1_4 = tf.layers.conv2d_transpose(inputs = output_1_4, filters = 128, kernel_size = 3, strides = (1,1), padding='same', data_format='channels_last', activation = tf.nn.relu)
	# output_1_5 = residual_block(inputs_layer = output_1_4, nb_blocks = 1, out_filters = 32, strides = 2)
	output_1_5 = tf.layers.conv2d_transpose(inputs = output_1_4, filters = 32, kernel_size = 3, strides = (2,2), padding='same', data_format='channels_last', activation = tf.nn.relu)
	...
	output_1_5 = tf.layers.conv2d_transpose(inputs = output_1_5, filters = 64, kernel_size = 3, strides = (1,1), padding='same', data_format='channels_last', activation = tf.nn.relu)
	# output_1_6 = residual_block(inputs_layer = output_1_5, nb_blocks = 1, out_filters = 16, strides = 2)
	output_1_6 = tf.layers.conv2d_transpose(inputs = output_1_5, filters = 16, kernel_size = 3, strides = (2,2), padding='same', data_format='channels_last', activation = tf.nn.relu)
	...
	output_1_6 = tf.layers.conv2d_transpose(inputs = output_1_6, filters = 32, kernel_size = 3, strides = (1,1), padding='same', data_format='channels_last', activation = tf.nn.relu)
	# output_1_7 = residual_block(inputs_layer = output_1_6, nb_blocks = 1, out_filters = 3, strides = 1)
	output_1_7 = tf.layers.conv2d_transpose(inputs = output_1_6, filters = 3, kernel_size = 3, strides = 1, padding='same', data_format='channels_last', activation = tf.nn.relu)
...
checkpoint_dir_1 = 'model_weight_rotation_v4.3/'
checkpoint_dir_2 = 'model_weight_rotation_v4.3/'
...
with tf.Session() as sess:
	...
	for i in range(50):
		...
		input_test = utils.load_image_com('/home/gaopeng/chair_only_trans_test_true/chair_source/chair_rotation_source_%.2d.png' % i)[:,:,:3]
		...
		# input_Semantic_test
		input_Semantic_test = np.loadtxt('/home/gaopeng/chair_only_trans_test_true/rendering_destination_rotation.txt')
		...
		# input_c_test
		input_c_test = utils.load_image_com('../../chair_only_trans_test_true/chair_rotation_true/' + 'chair_rotation_destination_%.2d.png' % i)[:,:,:3]
		...
		misc.imsave('../../chair_only_trans_test_true/chair_trans_test/chair_trans_test_%.2d.png' % i , output_test_image)