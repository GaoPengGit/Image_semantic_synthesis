...
def generator(path):
	...
	for i in range(10000):
		...
		# input_a = input_a[:,:,:3]
		# input_a = input_Sym.reshape((-1))
		# input_a = np.tile(input_a, 1)
		input_a = input_Sym.reshape((1, 224, 224, 3000))
		# receive input_b
		...
		input_b = input_b[(i*1):(i*1+1),:]
		# receive input_c
		# input_c = utils.load_image_com(path + 'chair_rotation_%.2d/chair_rotation_%.2d_trans_%.2d.png' % (j, j, i*8))[:,:,:3]
		input_c = utils.load_image_com(path + 'chair_rotation_00_only_%.2d.png' % (i*1))[:,:,:3]
		...
		# 	input_c = np.concatenate((input_c, utils.load_image_com(path + 'chair_rotation_00_only_%.2d.png' % (i*1+k))[:,:,:3]), axis = 0)
		input_c = np.reshape(input_c, (1, 224, 224, 3))
		# generate data
...
def VGG16_Synthesis_Network(input_img, input_Semantic):
	...
	output_1_1 = residual_block(inputs_layer = z_1, nb_blocks = 1, out_filters = 512, strides = 2)
...
checkpoint_dir_1 = 'model_weight_rotation_v3.0/'
checkpoint_dir_2 = 'model_weight_rotation_v3.0/'
